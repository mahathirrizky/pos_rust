...
const confirmDeleteSupplier = (supp) => {
    confirm.require({
        message: `Are you sure you want to delete ${supp.name}?`,
        ...
        <DataTable v-model:selection="selectedSuppliers" :value="supplierStore.suppliers" responsiveLayout="scroll">
          ...
          <Column headerStyle="width: 10rem">
             <template #body="slotProps">
                <Button icon="pi pi-pencil" class="mr-2 p-button-rounded p-button-success" @click="editSupplier(slotProps.data)" />
                <Button icon="pi pi-trash" class="p-button-rounded p-button-danger" @click="confirmDeleteSupplier(slotProps.data)" />
            </template>
          </Column>
        </DataTable>
      </template>
      ...
      <div class="p-grid p-fluid grid-nogutter">
        <div class="field col-12 p-2">
            <FloatLabel variant="on">
                <InputText id="name" v-model.trim="supplier.name" required="true" autofocus :class="{'p-invalid': submitted && !supplier.name}" variant="filled" fluid />
        ...
        <div class="field col-12 p-2">
            <FloatLabel variant="on">
                <InputText id="contact_person" v-model.trim="supplier.contact_person" variant="filled" fluid />
                <label for="contact_person">Contact Person</label>
            </FloatLabel>
        </div>
        <div class="field col-12 p-2">
            <FloatLabel variant="on">
                <InputText id="email" v-model.trim="supplier.email" required="true" :class="{'p-invalid': submitted && !supplier.email}" variant="filled" fluid />
        ...
        <div class="field col-12 p-2">
            <FloatLabel variant="on">
                <InputText id="phone" v-model.trim="supplier.phone" variant="filled" fluid />
                <label for="phone">Phone</label>
            </FloatLabel>
        </div>
        <div class="field col-12 p-2">
            <FloatLabel variant="on">
                <Textarea id="address" v-model="supplier.address" rows="3" cols="20" variant="filled" fluid />
